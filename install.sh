...
install-dependencies() {
  ...
  apt update > /dev/null
  apttmp=""
  for i in $( echo "nano vim mc iptables nginx python3 bc expect v4l-utils gpiod dialog git python3-pip python3-dbus-next python3-aiohttp python3-zstandard tesseract-ocr tesseract-ocr-chi-sim jq libxkbcommon0" )
  do
    apttmp=$apttmp" $i"
  done
  echo "apt install -y $apttmp"
  ...
  if [ ! -e /usr/bin/ustreamer ]; then
     if [[ $USE_USTREAMER -eq 1 ]]; then
       apt install -y ustreamer
     else  
       cd /tmp/
       apt install -y libevent-2.1-7 libevent-core-2.1-7 libevent-pthreads-2.1-7 build-essential
